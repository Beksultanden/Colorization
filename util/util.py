...
def rgb2xyz(rgb):  # rgb from [0,1]
    ...
    if rgb.dim() == 3:  # Если тензор имеет размерность [channels, height, width]
        rgb = rgb.unsqueeze(0)  # Добавляем размерность батча

    # Маска для преобразования значений
    mask = (rgb > .04045).type(torch.FloatTensor)
    if rgb.is_cuda:
        mask = mask.cuda()

    # Преобразование RGB в линейное пространство
